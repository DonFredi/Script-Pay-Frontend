 ...
 */
// The proxy uses Web Fetch primitives (Request/Response/Headers) from
...
describe("proxy", () => {
  ...
  it("lets a protected non-admin route through with only an expired access token plus a refresh token", async () => {
    ...
    const res = await proxy(makeRequest("/settings", { access_token: expired, refresh_token: "refresh-abc" }));
    expect(isPassThrough(res)).toBe(true);
  });
