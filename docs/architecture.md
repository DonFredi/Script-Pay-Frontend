...
Next.js 16 (App Router) — this repo
  ...
  ├─ Server: SSR/route handlers call the backend's absolute URL directly (no cookie problem there)
  └─ Client: axios (api-client.ts) → /api/backend/* (same-origin proxy, see next.config.ts) → backend
              │
              ▼
...
- `app/api/sentry-example-api` — the only real Next.js API route in this app; everything else the browser calls goes through the `/api/backend/:path*` catch-all rewrite in `next.config.ts`

## Auth flow

This app does **not** manage sessions itself — it verifies and relays tokens issued by the backend.

1. **Login/signup**: the browser calls `POST /api/backend/auth/login` (axios's `baseURL` is `/api/backend` client-side; `next.config.ts` rewrites `/api/backend/:path*` to the backend's real `:path*`, so this reaches `/auth/login`) and gets back `{ user, accessToken }` plus `access_token`/`refresh_token`/`csrf-token` cookies (set by the backend, httpOnly except the CSRF one). `useLogin`'s `onSuccess` calls `AuthProvider`'s `setSession(user, accessToken)`, which stores the access token **in memory only** (a module-level variable in `api-client.ts`) — never `localStorage`, to limit XSS blast radius.
